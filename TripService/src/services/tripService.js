...
import BookTripClient from '../rabbitmq/bookTrip/client'
import DriverInfoClient from '../rabbitmq/driver/driverInfo/client'
import UserInfoClient from '../rabbitmq/user/userInfo/client'
import { BroadCastUserClient } from '../rabbitmq/broadCast/broadCastUser/client'
import { BroadCastCallcenterClient } from '../rabbitmq/broadCast/broadCastCallcenter/client'
import CreateUserClient from '../rabbitmq/user/createUser/client'
import RealtimeClient from '../rabbitmq/realTime/client'
...
const getBasicTripInfo = async (trip_id) => {
...
const CreateTrip = async (data) => {
    return new Promise(async (resolve, reject) => {
        ...
        await BookTripClient.produce(trip)
...
const AcceptTrip = async (data) => {
    ...
    const driverInfo = await DriverInfoClient.produce(data.driver_id)
    // const driverInfo = await driverServices.GetDriverInfoById(data.driver_id)
    const returnDat = {
        trip_id: newTrip.trip_id,
        driverInfo,
    }
    await BroadCastUserClient.produce(newTrip.user_id, "found-driver-schedule", returnDat)
    // broadCastToClientById(newTrip.user_id, "found-driver-schedule", returnDat)
    const userInfo = await UserInfoClient.produce(newTrip.user_id)


    await FirebaseMessageClient.produce(userInfo.token_fcm, 'Chuyến đi hẹn giờ', "Tài xế đang đến chỗ bạn")
...
export const initTripForCallcenter = async (data) => {
    const phone = data.phone
    const user = await CreateUserClient.produce(phone)
    ...
    if (lat !== undefined && lng !== undefined) {
        const result = await db.Trip.findOne({
            ...
            attributes: {
                ...
                exclude: ['createdAt', 'updatedAt', "start", "end"]
            },
            raw: true,
            nest: true,
        })
        const userInfo = await UserInfoClient.produce(user_id)
        ...
        await BookTripClient.produce(trip)
        // SendMessageToQueue("callcenter-trip-queue", JSON.stringify(trip))
        await BroadCastCallcenterClient.produce("s3-trip", result)
        ...
        await BroadCastCallcenterClient.produce("s2-trip", trip2)
...
export const initTripCallCenterS2 = async (data) => {
    ...
    console.log("log result ở đây ", result)
    await BroadCastCallcenterClient.produce("s3-trip", result)
    // sendMessageToS3(result)
    // console.log("send trip to callcenter trip queue")
    await BookTripClient.produce(newTrip)
...
export const GetTripS2 = async () => {
    const result = await db.Trip.findAll({
        ...
        //     required: true,
        // },
        attributes: ["id", "type", 'createdAt', [db.Sequelize.json('start.place'), 'startAddress'],],
        order: [
            ['createdAt', 'DESC'],
        ],
        raw: true,
    ...
    if (result) {
        for (const trip of result) {
            const userInfo = await UserInfoClient.produce(user_id)
            trip.user = userInfo
            trip.user_phone = trip.user.phone
            trip.start = JSON.parse(trip.start)
            // trip.end = JSON.parse(trip.end)
        }
        return result
...
export const GetTripS3 = async () => {
    ...
    if (result) {
        ...
        for (const item of result) {
            ...
            if (item.driver_id) {
                const driverDat = await DriverInfoClient.produce(item.driver_id)
                item.driver = driverDat.driver_info
                item.driver_stats = driverDat.statics
            }
            if (item.start) {
...
export const GetRunningTripOfUser = async (user_id) => {
    ...
    if (trip) {
        for (const t of trip) {
            ...
            t.user = await UserInfoClient.produce(t.user_id)
            // const driverDat = await driverServices.GetDriverInfoById(t.driver_id)
            if (t.driver_id) {
                const driverDat = await DriverInfoClient.produce(t.driver_id)
...
//running
export const GetUserCurrentTrip = async (user_id) => {
    ...
    if (trip) {
        ...
        if (trip.driver) {
            const { location } = await RealtimeClient.produceDriverLocation(trip.driver_id)
            console.log("location là ", location)
...
const GetDriverCurrentTrip = async (driver_id) => {
    const t = await db.Trip.findOne({
        where: {
            driver_id: driver_id,
            is_scheduled: false,
            [Op.and]: [
