...
var updateLocationLoop = function updateLocationLoop(io) {
  ...
  setTimeout(function () {
    return updateLocationLoop(io);
  }, 60000);
};
var deleteTripExceeded = function deleteTripExceeded(io) {
  setInterval(function () {
    ...
    trips.forEach(function (trip_value, trip_id) {
      if (now - trip_value.createdAt.getTime() >= 300000 && trip_value.status === 'Pending' && trip_value.driver_id == null) {
        io["in"]("/trip/".concat(trip_id)).emit('trip-cancelled', {
          message: "trip cancelled due to lack of driver"
        });
...
var notifyIfClose = function notifyIfClose(io) {
  // for (const [trip_id,trip_value] of trips){
  ...
  trips.forEach(function (trip_value, trip_id) {
    if (locationService_1["default"].getDistance(trip_value.start.lat, trip_value.start.lng, trip_value.end.lat, trip_value.end.lng) <= 1) {
      io["in"]("/trip/".concat(trip_id)).emit('driver-close', {
        message: "driver is close"
      });
    }
  });
  setTimeout(function () {
    return notifyIfClose(io);
  }, 15000);
};
var runSocketService = function runSocketService(io) {
  initSocket(io);
  initSocketService(io);
  console.log("socket service started");
};
var initSocketService = function initSocketService(io) {
  updateLocationLoop(io);
  deleteTripExceeded(io);
  notifyIfClose(io);
};
var initSocket = function initSocket(io) {
...
var handleUserFindTrip = function handleUserFindTrip(socket) {
  socket.on('user-find-trip', function (data) {
...
var handleCallcenterFindTrip = function handleCallcenterFindTrip(socket) {
  socket.on('callcenter-find-trip', function (data) {
    return __awaiter(void 0, void 0, void 0, function () {
...
var handleDriverResponseBooking = function handleDriverResponseBooking(io, socket) {
  socket.on('driver-response-booking', function (data) {
    return __awaiter(void 0, void 0, void 0, function () {
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            ...
            trip_id = data.trip_id;
            if (data.status == 'Deny') return [2 /*return*/];
          ...
          case 1:
            ...
            if (updatedTrip === undefined) return [2 /*return*/];
...
var getCurrentDriverInfoById = function getCurrentDriverInfoById(id) {
...
var GetCurrentTripOfUser = function GetCurrentTripOfUser(id) {
  trips.forEach(function (trip_value, trip_id) {
    if (trip_value.user_id == id || trip_value.driver_id == id) {
      return trip_id;
    }
...
var GetDriversAround3KM = function GetDriversAround3KM(data) {
  var lat1 = data.lat;
  var lng1 = data.lng;
  var posDrivers = [];
  // for (const [driver_id,driver_value] of drivers) {
